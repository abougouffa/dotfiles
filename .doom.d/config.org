...
*** Registering Org Roam protocol :roam:
...
Exec=emacsclient %u
Icon=/home/abdelhak/.doom.d/assets/org-roam.png
Type=Application
Terminal=false
...
using the command-line app to show some meaningful diff information for some
binary files.
...
# Get the lastest version (uses html2text from 'python-html2text')
TIKA_VERSION=$(
  curl -s ${TIKA_BASE_URL} | # Get the page
  html2text --ignore-images --ignore-emphasis --ignore-links | # Convert page to HTML without images, emphasis and links.
