...
** TODO Realgud :realgud:
Doom's =:tools debugger= module installs =realgud= by default. The package needs to
be customized as I didn't found it useful over the builtin GUD! Maybe I didn't
get the point of this package, but it seems like it is less mature. I think the
builtin stuff + =emacs-gdb= gives a good experience for C/C++ programming.
...
*** TODO Mozilla =rr=
Add a shortcut to subsitite ="gdb"= command with ="rr"=, see
...
(defun rr-replay-cmd (prg mode)
  (let ((cmd (cond
              ((and (equal mode :gud) (boundp 'gud-gud-gdb-command-name)) gud-gud-gdb-command-name)
              ((and (equal mode :gdb-mi) (boundp 'gdb-executable-name)) gdb-executable-name)
              (t ""))))
