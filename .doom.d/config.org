...
** COMMENT Literate programming (=org-babel=) :babel:literate:
...
* Applications :apps:
** COMMENT Browser (=eww=) :eww:browser:
Set urls to open in a specific browser (=eww= here).

*** Display webpages in =eww=.
#+begin_src emacs-lisp
...
** COMMENT Emacs Multi-Media System (=emms=) :emms:music:
#+begin_src emacs-lisp
(emms-all)
...
(setq +mu4e-gmail-accounts '(("abdelhak.alg@gmail.com" . "/gmail")))
#+end_src
