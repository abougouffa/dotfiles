...
;; (unless (or (string-match-p "^Power N/A" (battery)) (string-match-p "unknown" (battery)))
;;   (display-battery-mode 1)) ; it's nice to know how much power you have
(setq ~some-text "the lazy dog jumps over the quick brown fox")
(unless (or (string-match-p "cat" ~some-text)
            (string-match-p "dog" ~some-text))
  (message "No DOGS no CATS"))
#+end_src
...
#+begin_src emacs-lisp
(setq doom-font (font-spec :family "JuliaMono" :size 30)
      doom-variable-pitch-font (font-spec :family "JuliaMono" :size 30)
...
;;(setq doom-theme 'nano-dark) ; Load theme
;;(load-theme 'nano t) ; Load theme
#+end_src
...
#+begin_src emacs-lisp
(package! auctex)
;;  :pin "6440ec5964dcbe58155e28f00f84ec0118d8fb7b")
#+end_src
...
(setq projectile-project-search-path '("~/PhD/workspace" "~/PhD/workspace-no" "~/Projects/foss_projects"))
...
** COMMENT Theme magic :theme:magic:terminal:
...
** COMMENT Treemacs :tree:treemacs:
...
#+end_src
*** COMMENT Usability enhancements
#+begin_src emacs-lisp
(after! elfeed-search
...
**** COMMENT Rebuild mail index while using mu4e
#+begin_src emacs-lisp
(after! mu4e
...
**** COMMENT Config transcoding & service management
...
#+begin_src emacs-lisp
(after! deft
  :custom (setq deft-directory "~/Dropbox/Org/slip-box/"
                deft-recursive t
