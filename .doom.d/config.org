...
#+begin_src emacs-lisp
(after! lsp-mode
  (setq lsp-lens-enable nil) ; not working properly with ccls!
  ...
  ;; Tune LSP performances (https://emacs-lsp.github.io/lsp-mode/page/performance)
  (setq lsp-log-io nil))
#+end_src
...
#+begin_src emacs-lisp
(after! (:and (:or c-mode c++-mode) tramp)
 (require 'lsp-mode)
 (setq lsp-enable-snippet nil)
 ;; (setq lsp-log-io t)
...
(after! dap-mode
  ...
  ;; Reduce the intrusive UI components to locals and tooltip (see ).
  (setq dap-auto-configure-features '(locals tooltip))

  ;; Hide the annoying server output
  (setq dap-auto-show-output nil)

  ;; Automatically trigger dap-hydra when a program hits a breakpoint.
  (add-hook 'dap-stopped-hook (lambda (arg) (call-interactively #'dap-hydra)))

  ;; Automatically delete session and close dap-hydra when DAP is ended.
  (add-hook 'dap-terminated-hook
    (lambda (arg)
      (progn (call-interactively #'dap-delete-session)
             (dap-hydra/nil)
             (message "DAP terminated, deleted current session.")))))
#+end_src
