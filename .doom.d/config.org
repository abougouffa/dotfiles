...
#+begin_src emacs-lisp
(setq doom-font (font-spec :family "Iosevka Fixed" :size 33)
      doom-variable-pitch-font (font-spec :family "Inter") ; inherits the :size from doom-font
...
| File                                 | Preview                                |
|--------------------------------------+----------------------------------------|
| =emacs-e-big.svg=                      | [[./assets/emacs-e-big.svg]]               |
|--------------------------------------+----------------------------------------|
| =gnu-emacs-logo-flat-white.svg=        | [[./assets/gnu-emacs-logo-flat-white.svg]] |
|--------------------------------------+----------------------------------------|
| =assets/gnu-emacs-logo-flat-light.svg= | [[./assets/gnu-emacs-logo-flat-light.svg]] |
|--------------------------------------+----------------------------------------|
| =assets/blackhole-lines-small.svg=     | [[./assets/blackhole-lines-small.svg]]     |
...
** EVIL multi cursors =evil-mc=
Doom Emacs define a set of =evil-mc-*= commands under =gz=, I like the default
bindings, but I miss =evil-mc-skip-and-goto-next-match= and
...
(unpin! spell-fu) ;; To use the new multi-language feature!
#+end_src
   ...
   (lambda ()
     ...
     (spell-fu-dictionary-add
      (spell-fu-get-personal-dictionary
       "en" (expand-file-name ".aspell.en.pws" doom-private-dir)))
     (spell-fu-dictionary-add
      (spell-fu-get-personal-dictionary
       "fr" (expand-file-name ".aspell.fr.pws" doom-private-dir))))))
#+end_src
...
rename the =[Gmail]/...= folders). Here an example:

#+begin_src conf-space :tangle no
# mbsync config file

# To avoid the "Warning: lost track of X pushed message" error
# https://www.reddit.com/r/emacs/comments/myxgrs/mu4e_mbsync_warning_lost_track_of_x_pushed/
CopyArrivalDate yes
...
# "gmail-inbox", "gmail-sent", "gmail-trash"...
Group gmail
Channel gmail-inbox
Channel gmail-sent
Channel gmail-trash
...
#+begin_src emacs-lisp
(add-hook 'lsp-mode-hook (lambda () (set-fringe-mode '(32 . 32))))
#+end_src
  ...
  (lsp-register-client
   (make-lsp-client
    ...
    (lsp-tramp-connection
     (lambda () (cons "pyright-langserver"
...
Add shortcuts to run =rr= from Emacs, the =rr record= takes the program name and
arguments from my local =ab/realgud:gdb-launch-plist=, when =rr replay= respects the
arguments configured in RealGUD's GDB command name.
