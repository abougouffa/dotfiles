...
#+options: title:t author:t email:t toc:t tags:nil
...
(setq doom-font (font-spec :family "Iosevka Fixed" :size 16)
      doom-variable-pitch-font (font-spec :family "Roboto") ; inherits the :size from doom-font
...
Integrate =git-delta= with =magit=.
...
This package is not maintained, but still a good for my Yocto projects. I don't
really use the =bitbake= commands from Emacs, so this package can be easily
replaced with [[https://bitbucket.org/olanilsson/bitbake-modes][~bitbake-modes~]].
...
time using =colcon build=, the repo contains example Docker files for [[https://github.com/DerBeutlin/ros.el/blob/master/docker/Dockerfile_noetic][Noetic]] and
[[https://github.com/DerBeutlin/ros.el/blob/master/docker/Dockerfile_foxy][Foxy]].
...
(use-package! ros
  ...
  (setq ros-workspaces
        (list (ros-dump-workspace
               :tramp-prefix (format "/docker:hacko@%s:" "916e561b4287")
               :workspace "~/phd_ws"
               :extends '("/opt/ros/noetic/"))
              (ros-dump-workspace
               :tramp-prefix (format "/ssh:swd_sk@%s:" "192.168.0.148")
               :workspace "~/ros_ws"
               :extends '("/opt/ros/noetic/"))
              (ros-dump-workspace
               :tramp-prefix (format "/ssh:swd_sk@%s:" "192.168.0.148")
...
UI components to provide a less intrusive UI, however I like to enable some less intrusive, more useful UI stuff.
...
=dap-mode=. I like also to have a mode minimal UI. I like to trigger =dap-hydra=
when the program hits a breakpoint, and automatically delete the session and
...
** The Grand "Cathedral" Debugger :realgud:
...
*** Additional commands
Using a pure GDB makes things more flexible, I also use =rr= (by substituting =gdb=
by =rr replay= when starting the session), which gives me the possibility to do
reverse debugging.
...
I do a lot of development on C/C++ apps which gets data from command line
arguments, which means I have to type my arguments manually after calling
=realgud:gdb=, which is very annoying.

For DAP mode, there is support for either =dap-debug-edit-template=, or
...
(defvar ab/realgud:gdb-launch-plist nil)
#+end_src
...
((nil . ((ab/realgud:gdb-launch-plist . '(:program "${workspaceFolder}/build/bin/my_prog"
...
The special variable =${workspaceFolder}= will be replaced by the project's root
(from =projectile-project-root=).

#+begin_src emacs-lisp
(cl-defun ab/realgud:gdb-get-launch-cmd (&key program args)
  (require 'realgud)
  (let ((gdb-cmdline realgud:gdb-command-name))
    (when program
      (setq gdb-cmdline (s-join " " (list gdb-cmdline "--args" program)))
      (when args
        (setq gdb-cmdline (s-join " " (list gdb-cmdline (s-join " " args))))))
    ;; Replace special variables
    (message (format "gdb-cmdline: %s" gdb-cmdline))
    (s-replace "${workspaceFolder}" (or (projectile-project-root) ".") gdb-cmdline)))

(defun +debugger/realgud:gdb-launch ()
  "Launch RealGUD with parameters from `ab/realgud:gdb-launch-plist'"
  (interactive)
  (if ab/realgud:gdb-launch-plist
    (progn
      (realgud:gdb (apply 'ab/realgud:gdb-get-launch-cmd ab/realgud:gdb-launch-plist))
      (message (format "`ab/realgud:gdb-launch': Calling `realgud:gdb' with args: `%s'"
                       (apply 'ab/realgud:gdb-get-launch-cmd ab/realgud:gdb-launch-plist))))
    (progn
      (message "Variable `ab/realgud:gdb-launch-plist' is `nil'")
      (realgud:gdb))))
...
*** TODO Mozilla =rr=
Add a shortcut to substitute ="gdb"= command with ="rr"=.
Some useful hints could be found [[http://notes.secretsauce.net/notes/2017/02/24_interfacing-rr-to-gdb-in-gnu-emacs.html][here]], [[https://github.com/rr-debugger/rr/blob/82ab0f9f5bc13fb27acaccb9a85397bcaa7a7b05/src/ReplayCommand.cc#L45][here]], [[https://github.com/rr-debugger/rr/issues/1484][here]] and [[https://emacs.stackexchange.com/questions/20056/is-it-possible-to-use-mozillas-rr-with-gdb-multi-window][here]].

#+begin_src emacs-lisp :tangle no
;; NOTE: Not tangled, WIP
(after! gud
  (require 's)

  (defun rr-replay-cmd (prg mode)
    (let ((cmd (cond ((and (equal mode :gud) (boundp 'gud-gud-gdb-command-name)) gud-gud-gdb-command-name)
                     ((and (equal mode :emacs-gdb) (boundp 'gdb-executable-name)) gdb-executable-name)
                     ((and (equal mode :gdb-mi) (boundp 'gud-gdb-command-name)) gud-gdb-command-name)
                     (t ""))))
      (s-replace "gdb" "rr replay" cmd)))

  (defun rr-record-cmd (prg)
    (concat "rr record " prg))

  ;; Tests
  (rr-record-cmd "my_prog")
  (rr-replay-cmd "my_prog" :gdb-mi))
#+end_src
...
**** COMMENT Font trick
Org files can be rather nice to look at, particularly with some
...
Let's start by adding a sans font. I'd also like to use some
