...
This needs to be dropped in =/etc/NetworkManager/dispatcher.d/99-wlan=:

#+begin_src shell :tangle "/sudo::/etc/NetworkManager/dispatcher.d/99-wlan" :mkdirp yes :tangle-mode (identity #o755)
#!/bin/bash

# For more information about NetworkManager's dispatcher scripting, see: https://networkmanager.dev/docs/api/latest/NetworkManager-dispatcher.html

wired_interfaces="en.*|eth.*"
interface="$1"
action="$2"

echo "Network change datected -- interface: ${interface} -- action: ${action}"

if [[ "${interface}" =~ ${wired_interfaces} ]]; then
    case "${action}" in
        up)
            echo "Ethernet interface ${interface} is up, disabling WiFi"
            nmcli radio wifi off
            ;;
        down)
            echo "Ethernet interface ${interface} is down, enabling WiFi"
            nmcli radio wifi on
            ;;
    esac
fi
