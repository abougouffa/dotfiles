...
This repository ([[https://github.com/abougouffa/dotfiles][abougouffa/dotfiles]]) contains my configuration files for *Bash*,
*Zsh*, *MinEmacs*, *Neovim*, *Alacritty*, *Ghostty* and other Linux related stuff.
...
configuration files (Bash configuration, define aliases, environment variables,
user tools, Git configuration...).

* Deploying the dotfiles :deploy:stow:
To deploy these dotfiles, I'm using the simple GNU Stow. The configuration files
(including the ones tangled from the =literate-config.org= file) are stored in
seperate directories. For example: Bash and Zsh configuration files are stored
in =shell/=, GDB configuration files in =gdb/=, Emacs/MinEmacs configuraiton files
in =emacs/=, and so on.
...
To deploy a configuration using GNU Stow, it is as simple as running the
following commands:
...
* Introduction :intro:
I've been using Linux exclusively since 2009, *GNU Emacs* was always installed on
my machine, but I didn't discover the *real* Emacs until 2020. In the beginning, I
started my vanilla Emacs configuration from scratch, but after a while, it had
become a real mess. As a new Emacs user, I didn't understand in the beginning
how to optimize my configuration and how to do things correctly, which resulted
in a configuration that takes more than 30s to load.

I discovered then [[https://github.com/syl20bnr/spacemacs][Spacemacs]], which made things much easier, but it was a little
slow and it was hiding a lot of Emacs features behind some /simplistic/
configuration options, which isn't perfect for someone who loves writing code.
Just after, I found the awesome [[https://github.com/doomemacs/doomemacs][Doom Emacs]], which helped me progress and learn
more about Emacs and Elisp, but at some point, I faced [[https://abougouffa.github.io/minemacs/docs/STORY.html][multiple problems with
...
I planned to use only a small set of necessary packages (hence, the /Min/ in
/MinEmacs/). However, it is starting to grow to respond to my daily needs.

This block will be tangled in the file =bootstrap.sh=, which will be used as a
script to conditionally install and configure some system features.
...
#+name: personal-laptop-p
#+begin_src elisp :exports none
(if (equal (system-name) "laptop") "true" "false")
#+end_src
...
This combo should speedup opening files, for right-to-left languages like
Arabic, you might need to disable this or set it in a per-buffer basis.
...
*** Awqat
Awqat (Arabic: أوقات meaning /Times/), is a package to calculate Islamic prayer
times based on the current location.
...
**** Offline dictionaries for =lexic= and =sdcv=
The =lexic= package needs ~sdcv~ to be installed, it needs also some /StarDict/
dictionaries to be installed, see =M-x lexic-dictionary-help=. You can download
some dictionaries from:

- [[https://web.archive.org/web/20230325055703/http://download.huzheng.org/dict.org][Dict.org archive]]
- [[https://web.archive.org/web/20230325180504/http://download.huzheng.org/bigdict][Big English dictionaries]]
- [[https://polyglotte.tuxfamily.org/doku.php?id=donnees:dicos_bilingues][French dictionaries]]
...
As specified in Org-roam [[https://www.orgroam.com/manual.html#org_002droam_002dprotocol][official documentation]], we would like to invoke the
...
A wrapper around =emacsclient=, adapted from [[https://tecosaur.github.io/emacs-config/config.html#emacs-client-wrapper][tecosaur's Emacs configuration]].
...
#+begin_src shell :tangle shell/.zshrc :noweb yes
# -*- mode: sh -*-
...
#+begin_src shell :tangle shell/.bashrc :noweb yes
# -*- mode: sh -*-
...
#+begin_src gitconfig :noweb yes :tangle "git/.config/git/config" :mkdirp yes
[commit]
  gpgsign = <<personal-laptop-p()>>
#+end_src
...
Mergiraf is a Tree-sitter based tool to help solve merge conflicts while taking
into account the language's syntax (using Tree-sitter to parse the files and
compare the ASTs). We add =mergiraf= as a merge tool in Git, then we make =mergiraf=
the default tool for supported extensions.

#+begin_src gitconfig :tangle (if (executable-find "mergiraf") "git/.config/git/config" "no") :mkdirp yes
[merge "mergiraf"]
    name = mergiraf
    driver = mergiraf merge --git %O %A %B -s %S -x %X -y %Y -p %P -l %L
#+end_src

#+name: mergiraf-gitattributes
#+begin_src sh :results output
mergiraf languages --gitattributes
#+end_src

#+begin_src gitconfig :noweb yes :tangle (if (executable-find "mergiraf") "git/.config/git/attributes" "no") :mkdirp yes
# Use Mergiraf for supported languages
<<mergiraf-gitattributes()>>
#+end_src
...
#+begin_src shell :tangle git/.local/bin/git-replace-email :mkdirp yes :tangle-mode (identity #o755) :comments no
#!/usr/bin/env bash
...
*** =git ignore=
Let's have =git ignore <lang>= to automatically generate =.gitignore= patterns for a
specific language.
...
compressed files like ~*.bz2~, ~*.apk~... etc. As a workaround, we disable this
feature by exporting an XML config file which will be used when launching the
Tika App (using ~--config=<tika-config.xml>~).
...
** Ollama
Add shell completion for Ollama, stoken from [[https://github.com/ollama/ollama/issues/1653#issuecomment-2184527185][Adam Niederer's comment]].

#+begin_src bash :tangle shell/.bashrc
_complete_ollama() {
...
** Disable WiFi when wired network is available

This needs to be dropped in =/etc/NetworkManager/dispatcher.d/99-prefer-wired-connection=:

Inspired by https://www.matoski.com/article/wifi-ethernet-autoswitch, For more
information about NetworkManager's dispatcher scripting, check:
https://networkmanager.dev/docs/api/latest/NetworkManager-dispatcher.html.

#+begin_src shell :tangle (if (equal (system-name) "laptop") "/sudo::/etc/NetworkManager/dispatcher.d/99-prefer-wired-connection" "no") :mkdirp yes :tangle-mode (identity #o755)
#!/usr/bin/env bash

syslog_tag="prefer-wired-connection"

iface="$1"
action="$2"

if [[ -n "${iface}" ]] && [[ -n "${action}" ]]; then
    iface_type="$(nmcli -t -f GENERAL.TYPE dev show "${iface}" | cut -d':' -f2)"
    iface_state="$(nmcli -t -f GENERAL.STATE dev show "${iface}" | sed -n 's/GENERAL\.STATE:[[:digit:]]*[[:space:]]*(\(.*\))/\1/p')"

    logger -i -t "${syslog_tag}" "Interface: ${iface} -- type: ${iface_type} -- state: ${iface_state} -- action: ${action}"

    if [ "${iface_type}" = "ethernet" ] && [ "${action}" = "down" ]; then
        logger -i -t "${syslog_tag}" "Ethernet interface ${iface} is down, enabling WiFi..."
        nmcli radio wifi on
    elif [ "${iface_type}" = "ethernet" ] && [ "${action}" = "up"  ] && [ "${iface_state}" = "connected" ]; then
        logger -i -t "${syslog_tag}" "Ethernet connection detected on interface ${iface}, disabling WiFi..."
        nmcli radio wifi off
    fi
fi
#+end_src

* Local variables :ignore:noexport:
