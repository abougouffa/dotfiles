...
*Zsh*, *MinEmacs*, *Neovim*, *Ghostty*, *Alacritty* and other Linux related stuff.
...
separate directories. For example: Bash and Zsh configuration files are stored
in =shell/=, GDB configuration files in =gdb/=, Emacs/MinEmacs configuration files
in =emacs/=, and so on.
